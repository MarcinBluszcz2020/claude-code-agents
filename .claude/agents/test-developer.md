...
- `framework`: context-appropriate (jest, pytest, etc.)
...
Agent Name: test-developer
Session ID: {session_id}
Report Path: project/internal/{session_id}/005-001-test-report.json
Status: success
Update Data:
  Files Modified: [list]
  Test Metrics:
    ...
    Mocks Created: {number}
```

## AGENT SIGNATURE PROTOCOL

Use the `agent-signature-generator` skill for ALL test files:
...
Agent Name: test-developer
Session ID: {session_id}
Purpose: Test suite for {module}
Requested By: {orchestrator}
```

The skill generates test-specific signatures with coverage targets and strategy notes.
