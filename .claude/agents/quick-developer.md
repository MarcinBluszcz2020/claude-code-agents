...
Update Data:
  ...
  Metrics:
    Lines Added: {number}
    Shortcuts Taken: {number}
  Notes: MVP implementation details
```
...
The skill automatically generates appropriate signatures for code files (.js, .py, .ts, .cs, etc.) and documentation (.md) with MVP-specific notes
